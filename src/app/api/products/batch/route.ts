  ...
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      try {
        ...
        if (!productData.name?.ar || !productData.name?.en || !productData.slug) {
          errors.push(`Product ${i + 1}: Missing required fields (name.ar, name.en, slug)`)
          continue
        }
        ...
        if (existingProduct) {
          errors.push(`Product ${i + 1}: Slug "${productData.slug}" already exists`)
          continue
        }
