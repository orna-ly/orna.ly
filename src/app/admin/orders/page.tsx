...
import { ordersAtom, currentLangAtom } from '@/lib/atoms'
...
export default function AdminOrdersPage() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'NEW': return <Clock className="h-4 w-4 text-orange-500" />
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder={currentLang === 'ar' ? 'البحث برقم الطلب أو اسم العميل...' : 'Search by order number or customer name...'}
                  className="pl-10"
                />
              </div>
            </div>
            <Select>
              <SelectTrigger className="w-full md:w-48">
                <SelectValue placeholder={currentLang === 'ar' ? 'حالة الطلب' : 'Order Status'} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{currentLang === 'ar' ? 'جميع الحالات' : 'All Status'}</SelectItem>
                <SelectItem value="NEW">{currentLang === 'ar' ? 'جديد' : 'New'}</SelectItem>
            ...
            <Select>
              <SelectTrigger className="w-full md:w-48">
                <SelectValue placeholder={currentLang === 'ar' ? 'حالة الدفع' : 'Payment Status'} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{currentLang === 'ar' ? 'جميع الحالات' : 'All Status'}</SelectItem>
        ...
        <CardContent>
          <div className="border rounded-lg overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{currentLang === 'ar' ? 'رقم الطلب' : 'Order ID'}</TableHead>
                  <TableHead>{currentLang === 'ar' ? 'العميل' : 'Customer'}</TableHead>
                  <TableHead>{currentLang === 'ar' ? 'المنتج' : 'Product'}</TableHead>
                  <TableHead>{currentLang === 'ar' ? 'المبلغ' : 'Amount'}</TableHead>
              ...
              <TableBody>
                {orders.map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell>
                      <div>
                        <div className="font-medium">{order.product.name[currentLang]}</div>
                        <div className="text-sm text-neutral-500">
                          {currentLang === 'ar' ? 'الكمية:' : 'Qty:'} {order.quantity}
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge className={`${getStatusColor(order.status)} flex items-center gap-1 w-fit`}>
                        ...
                        {order.status === 'SHIPPED' && (currentLang === 'ar' ? 'تم الشحن' : 'Shipped')}
                        {order.status === 'DELIVERED' && (currentLang === 'ar' ? 'تم التوصيل' : 'Delivered')}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Badge className={getPaymentStatusColor(order.paymentStatus)}>
                        {order.paymentStatus === 'PAID' && (currentLang === 'ar' ? 'مدفوع' : 'Paid')}
                        {order.paymentStatus === 'PENDING' && (currentLang === 'ar' ? 'معلق' : 'Pending')}
                        {order.paymentStatus === 'FAILED' && (currentLang === 'ar' ? 'فشل' : 'Failed')}
                      </Badge>
                    </TableCell>
                    <TableCell>
